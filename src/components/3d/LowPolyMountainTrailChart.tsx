...
export const LowPolyMountainTrailChart: React.FC<LowPolyMountainTrailChartProps> = ({
  ...
  const {
    ...
    // Build 6 Realistic Layered Alpine Tundra, Lichen & Exposed Scree Terraces
    const terraceSpecs = [
      {
        startRow: 28,
        endRow: 38,
        lobes: 7,
        amp: 16,
        sunlitFill: '#4A5847',      // dark alpine tundra
        shadowFill: '#2D3E2D',      // dark forest
        outlineColor: '#1A2419',
      },
      {
        startRow: 22,
      ...
      // Higher zones get rockier
      {
        startRow: 11,
        endRow: 19,
        lobes: 5,
        amp: 13,
        sunlitFill: '#8B9D82',      // lichen/moss on rock
        shadowFill: '#6B7D72',
        outlineColor: '#5A6B57',
      },
      {
        ...
        lobes: 5,
        amp: 12,
        sunlitFill: '#9BADA2',      // exposed scree
        shadowFill: '#7B8D82',
        outlineColor: '#6B7D72',
      },
      {
        ...
        lobes: 4,
        amp: 10,
        sunlitFill: '#B5ADA0',      // bare rock/scree
        shadowFill: '#8B9D92',
        outlineColor: '#7B8D82',
      },
    ];
    ...
    // Realistic Weathered Rock Boulders & Cliff Outcrops
    ...
    // Realistic Alpine Tundra Grass Tufts & Subdued Wildflowers
    ...
    for (let r = 8; r <= numRows - 2; r += 3) {
      for (let c = 2; c <= numCols - 2; c += 3) {
        ...
        cartoonGrassTufts.push({
          ...
          flowerColor: flowerSeed > 0.82 ? '#D4AF8F' : '#8B4860',
        });
      }
    }
        ...
        >
          ...
          >
            <defs>
              ...
              {/* Rocky Noise for Cliff/Boulder Details */}
              <filter id="rockNoise">
                <feTurbulence
                  type="fractalNoise"
                  baseFrequency="0.08"
              ...
              {/* Weathered Surface (Subtle Bumps) */}
              <filter id="weatheredSurface">
                <feTurbulence type="fractalNoise" baseFrequency="0.15" numOctaves="3" />
                <feDisplacementMap in="SourceGraphic" scale="1.8" />
              </filter>

              <linearGradient id="stoneNoiseGradient" x1="0" y1="0" x2="1" y2="1">
                <stop offset="0%" stopColor={REALISTIC_PALETTE.rockLight} />
                <stop offset="50%" stopColor={REALISTIC_PALETTE.rockMid} />
                <stop offset="100%" stopColor={REALISTIC_PALETTE.rockDark} />
              </linearGradient>

              {/* Rough Stone Gradient + Noise Overlay */}
              <pattern
                id="roughStonePattern"
                patternUnits="userSpaceOnUse"
                width="80"
                height="80"
              >
                <rect width="80" height="80" fill={REALISTIC_PALETTE.rockMid} />
                <rect
                  width="80"
                  height="80"
                  fill="url(#stoneNoiseGradient)"
                  opacity="0.4"
                  filter="url(#rockNoise)"
                />
              </pattern>

              {/* Subtle Erosion Lines & Water Channels Pattern */}
              <pattern
                id="erosionPattern"
                patternUnits="userSpaceOnUse"
                width="60"
              ...
              >
                <path
                  d="M 15 0 Q 12 60 18 120 M 45 0 Q 48 60 42 120"
                  stroke={REALISTIC_PALETTE.rockDark}
                  strokeWidth="0.8"
                  fill="none"
                  opacity="0.3"
                />
              ...
              {/* Distant Realistic Peaks Gradients */}
              <linearGradient id="distantSunlitSnow" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#9A8E83" />
                <stop offset="50%" stopColor="#7B6E63" />
                <stop offset="100%" stopColor="#4A5847" />
              </linearGradient>
              <linearGradient id="distantShadowSnow" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#6B5D52" />
                <stop offset="50%" stopColor="#4E433B" />
                <stop offset="100%" stopColor="#2D3E2D" />
              </linearGradient>

              {/* Flanking Realistic Valley Hill Gradients */}
              <linearGradient id="flankHillLeftFar" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#5A6B57" />
                <stop offset="55%" stopColor="#4A5847" />
                <stop offset="100%" stopColor="#2D3E2D" />
              </linearGradient>
              <linearGradient id="flankHillRightFar" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#4A5847" />
                <stop offset="55%" stopColor="#3D4E3D" />
                <stop offset="100%" stopColor="#1A2419" />
              </linearGradient>
              <linearGradient id="flankHillLeftNear" x1="0" y1="0" x2="1" y2="1">
                <stop offset="0%" stopColor="#4A5847" />
                <stop offset="55%" stopColor="#2D3E2D" />
                <stop offset="100%" stopColor="#1A2419" />
              </linearGradient>
              <linearGradient id="flankHillRightNear" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#3D4E3D" />
                <stop offset="55%" stopColor="#2D3E2D" />
                <stop offset="100%" stopColor="#141C13" />
              </linearGradient>

              {/* Realistic Main Mountain Base Underpainting */}
              <linearGradient id="mainMassifSunlitBase" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor={REALISTIC_PALETTE.rockLight} />
                <stop offset="28%" stopColor={REALISTIC_PALETTE.scree} />
                <stop offset="65%" stopColor={REALISTIC_PALETTE.alpineTundra} />
                <stop offset="100%" stopColor={REALISTIC_PALETTE.forest} />
              </linearGradient>
              <linearGradient id="mainMassifShadowBase" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor={REALISTIC_PALETTE.rockMid} />
                <stop offset="30%" stopColor={REALISTIC_PALETTE.rockDark} />
                <stop offset="68%" stopColor={REALISTIC_PALETTE.forest} />
                <stop offset="100%" stopColor={REALISTIC_PALETTE.forestDark} />
              </linearGradient>
            ...
            >
              {distantSnowPeaks.map((pk, idx) => (
                <g key={`dist-snow-pk-${idx}`}>
                  ...
                  <path
                    d={pk.snowCapD}
                    fill={REALISTIC_PALETTE.snowWhite}
                    stroke={REALISTIC_PALETTE.scree}
                    strokeWidth="1.4"
                    strokeLinejoin="round"
                    filter="url(#weatheredSurface)"
                  />
                </g>
              ))}
            </g>
            ...
            {/* 5. Main Mountain Base Silhouette & Stone Texture */}
            <path
              d={mountainBaseHullD}
              fill="url(#roughStonePattern)"
              filter="url(#stoneTexture)"
              stroke="#3A3A3A"
              strokeWidth="2"
              strokeLinejoin="round"
            />
            <path d={mountainLeftSunlitD} fill="url(#mainMassifSunlitBase)" opacity="0.88" />
            <path d={mountainRightShadowD} fill="url(#mainMassifShadowBase)" opacity="0.88" />

            {/* 6. Layered Realistic Alpine Tundra, Moss & Scree Terraces */}
            <g>
              {cartoonHillTerraces.map((terrace, idx) => (
                <g key={`cartoon-terrace-${idx}`}>
                  ...
                  <path
                    ...
                    fill="none"
                    stroke={REALISTIC_PALETTE.rockLight}
                    strokeWidth="1.6"
                    strokeLinecap="round"
                    opacity="0.45"
                  />
            ...
            {/* Subtle Erosion Lines & Water Channels Overlay */}
            <path
              d={mountainBaseHullD}
            ...
            {/* 6B. Realistic Weathered Summit Snow-Cap Crown */}
            <g>
              <path
                d={cartoonSnowCap.fullD}
                fill={cartoonSnowCap.snowFill}
                stroke={REALISTIC_PALETTE.scree}
                strokeWidth="2"
                strokeLinejoin="round"
                filter="url(#weatheredSurface)"
              />
              <path
                d={cartoonSnowCap.shadowD}
                fill={cartoonSnowCap.shadowFill}
                opacity="0.85"
              />
              <path
                d={cartoonSnowCap.bottomWaveD}
                fill="none"
                stroke={cartoonSnowCap.rimColor}
                strokeWidth="1.8"
                strokeLinecap="round"
              />
            </g>

            {/* 6C. Central Mountain Ridge Line */}
            <path
            ...
            {/* 6D. Realistic Weathered Boulders & Alpine Tundra Grass Tufts */}
            <g>
              {cartoonBoulders.map((b, idx) => (
                ...
                >
                  ...
                  <path
                    ...
                    filter="url(#weatheredSurface)"
                  />
                  {/* Rock detail lines (cracks) */}
                  <path
                    d={`M ${(-b.rx * 0.6).toFixed(1)} ${(-b.ry * 0.4).toFixed(1)} Q 0 ${(-b.ry * 0.8).toFixed(
                      1
                    )} ${(b.rx * 0.5).toFixed(1)} ${(-b.ry * 0.2).toFixed(1)}`}
                    fill="none"
                    stroke="#6B5D52"
                    strokeWidth="1"
                    strokeLinecap="round"
                    ...
                    {tr.variant === 'pine' ? (
                      ...
                      <>
                        <circle
                          ...
                          strokeWidth="1"
                        />
                        <circle cx={-w * 0.28} cy={-h * 0.58} r={2.0 * tr.scale} fill="#8B4860" />
                        <circle cx={w * 0.24} cy={-h * 0.44} r={1.7 * tr.scale} fill="#7A3E54" />
                        <circle cx={0} cy={-h * 0.72} r={1.5 * tr.scale} fill="#D4AF8F" />
                      </>
                    )}
                  </g>
                );
